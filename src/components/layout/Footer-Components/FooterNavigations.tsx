...
export default function FooterNavigations() {
  const currentYear = new Date().getFullYear();
  return (
    <>
      <div className='px-6 md:px-15 bg-bg-primary pt-60 pb-4 w-full '>
        <div className=' flex flex-col-reverse md:flex-row md:justify-between w-[100%]'>
          <div className=' md:w-46 '>
            <h3 className='text-primary md:text-white text-center md:text-start text-1.5xl font-primary font-bold pb-5'>LOCATIONS</h3>

            {addressArray.map((address, index) => (
              <p key={index} className=' text-white text-center md:text-start text-base font-normal leading-6 w-full pb-5'>
                {address.content}
          ...
          <div className=' md:w-46 '>
            <h3 className=' text-primary md:text-white text-center md:text-start text-1.5xl font-primary font-bold pb-5'>SERVICES</h3>

            {servicesLinkData.map((link) => (
              <Link key={link.id} href={link.url} className={``}>
                <p
                  className={`text-white text-base text-center md:text-start font-normal leading-6  pb-2
            ...
            <hr className=' w-full md:hidden text-white opacity-20 mt-16' />
          </div>
          <div className=' md:w-46 '>
            <h3 className=' text-primary md:text-white text-center md:text-start text-1.5xl font-primary font-bold pb-5 '>SUPPORT</h3>
            {supportLinkData.map((link) => (
              <Link key={link.id} href={link.url}>
                <p className='text-white text-base text-center md:text-start font-normal leading-6 w-full pb-2'>{link.text}</p>
              </Link>
            ))}
            <hr className=' w-full md:hidden text-white opacity-20 mt-16' />
          </div>

          <div className=' bg-primary md:w-[35%]   rounded-[15px] flex flex-col mb-24 md:mb-0  items-center  px-8'>
            <h1 className=' text-white text-center text-1.7xl pt-[39px] '>Get in Touch With Us.</h1>

            <div className=' flex justify-between   pt-[36px] w-full '>
              <input
        ...
        <div className=' w-full pt-14 pb-[54px] flex justify-between items-center'>
